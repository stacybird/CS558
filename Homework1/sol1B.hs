...
exp_to_string (exp) =  
    case exp of
      ...
      Rem(e1,e2) ->  "(% " ++ (exp_to_string e1) ++ " " ++ (exp_to_string e2) ++ ")"
      Int n -> show n
...
compile (Rem(e1,e2)) =  (compile e1) ++ (compile e2) ++ [Divrem,Swap,Pop]
...
operatorP = plus <|> times <|> minus <|> divide <|> remainder
  where plus   = do { symbol xxx "+"; return Add}
        ...
        remainder = do { symbol xxx "%"; return Rem} -- find where this is to fix.
